...
enum ErrorCode {
    ...
    /** {zh}
     * @brief 不能找到指定的画布。  <br>
     *        通常发生在依赖画布设置的函数调用失败的时候。
     */
    /** {en}
     * @brief Unable to find the specified canvas.  <br>
     *        Usually occurs when a function call that relies on canvas not setting.
     */
    kErrorCodeCannotFoundCanvas = -1087,
    /**
     * @hidden for internal use only
     */
